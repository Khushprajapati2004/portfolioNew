...
function Skills() {
  ...
  const organizedSkills = useMemo(() => {
    ...
    apiSkills.forEach(skill => {
      ...
      categories[category].push(...names)
    })
    return categories
  }, [apiSkills])

  // Flat list of unique skills for the primary stack
  const primaryTech = useMemo(() => {
    const allNames = apiSkills.flatMap(s => s.name.split(',').map(n => n.trim()).filter(Boolean))
    return Array.from(new Set(allNames)).slice(0, 10)
  }, [apiSkills])
